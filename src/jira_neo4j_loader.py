...
JQL_QUERY = os.getenv("JQL_QUERY", 'summary ~ "CIPOE*" ORDER BY updated DESC')
    ...
    try:
        ...
        while total_fetched < max_issues:
            ...
            try:
                ...
                # Apply filtering: CIPOE prefix AND has at least one "is impacted by" link
                kept_issues = []
                for issue in issues:
                    title = (issue.get("fields", {}).get("summary") or "")
                    if is_cipoe_prefix(title):
                        impacted_keys = extract_impacted_by_keys(issue)
                        if len(impacted_keys) > 0:  # Must have at least one "is impacted by" link
                ...
                logger.info(f"Page: fetched {len(issues)}, kept {kept_count} (prefix+has-links), total so far: {total_kept_after_filter}")
        ...
        logger.info(f"Final totals - Fetched: {total_fetched}, Kept after filter: {total_kept_after_filter}")
        ...
        if all_rows:
            ...
            for i, row in enumerate(all_rows[:3]):
                ...
                logger.info(f"  {row['key']}: {summary} (#{len(row['impactedKeys'])} impacted)")
